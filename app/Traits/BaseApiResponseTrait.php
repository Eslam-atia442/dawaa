...
trait BaseApiResponseTrait
{
     ...
     * @return JsonResponse
     */
    public function errorWrongArgs($message = null , int $statusCode = Response::HTTP_BAD_REQUEST): JsonResponse
    {
        if (empty($message)) {
            $message = __('Wrong Arguments');
        }

        return $this->setStatusCode($statusCode)->respondWithError($message , $statusCode);
    }
