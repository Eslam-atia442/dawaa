...
abstract class BaseRepository implements BaseContract
{
    ...
    public function create(array $attributes = []): mixed
    {
        ...
        if (!empty($attributes)) {
            ...
            foreach ($this->model->filesToUpload ?? [] as $file) {
            ...
            foreach ($model->filesToUpload ?? [] as $file) {
                ...
                if ($files instanceof \Illuminate\Http\UploadedFile) {
                    // Single file upload
                    $model->addOptimizedMedia($files, $file, 25);
                } elseif (is_array($files)) {
                    // Handle array of files
                    foreach ($files as $f) {
                        if ($f instanceof \Illuminate\Http\UploadedFile) {

                            $model->addOptimizedMedia($f, $file, 25);
                        } else {
                            \Log::warning("Skipping invalid file upload", [
                                'field'      => $file,
                                'value_type' => gettype($f),
                                'value'      => $f,
                            ]);
                        }
                    }
                } else {
                    \Log::warning("Unexpected file input", [
                        'field'      => $file,
                        'value_type' => gettype($files),
                        'value'      => $files,
                    ]);
                }
            }
            ...
            if (in_array('App\Traits\HasWalletTraitTrait', class_uses($model))) {
                $model->createWallet();
            }
...
//            dd($attributes);
            ...
            foreach ($model->filesToUpload ?? [] as $file) {
                ...
                if ($files instanceof \Illuminate\Http\UploadedFile) {
                    // Single file upload - delete old files
                    $oldMedia = $model->getMedia($file);
     ...
     * @throws Exception
     */
    public function remove($models): mixed
    {
        // Check if has relations
        $models = (array)json_decode($models, true);
        $models = $this->query->whereIn('id', $models)->get();

        foreach ($models as $model) {
            foreach ($model->getRestrictedRelations() as $relation) {
                if ($model->$relation()->count()) {
                    // throw new CantDeleteModelException(__('trans.errors.cannot_delete_model_with_relations', ['model' => $this->modelName, 'relation' => $relation]));
                    throw new CantDeleteModelException(__('trans.errors.cannot_delete_model_with_relations2'));
    ...
    public function removeFiles($model)
    {
        return File::where('fileable_id', $model->id)->where('fileable_type', $this->modelName)->delete();
    }
...
//            info($query->toSql());
    ...
    {
        $collected = collect($attributes)->filter(function ($value, $key){
            return $this->model->isFillable($key);
        })->toArray();

        // remove eny attributes that are null
