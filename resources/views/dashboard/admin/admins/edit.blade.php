        ...
        <div class="row g-0">
            ...
            <div class="col-md-9">
                <div class="card-body p-4">
                    <form class="form validated-form" method="POST" action="{{route('admin.admins.update' , ['admin' => $row->id])}}" novalidate enctype="multipart/form-data">
                        @csrf
                        <div class="tab-content" id="v-pills-tabContent">
                            <div class="tab-pane fade show active" id="v-pills-general" role="tabpanel" aria-labelledby="v-pills-general-tab" tabindex="0">
                                <div class="row g-3">
                                    <x-admin.input required="true" :value="$row->name" name="name" label="name" type="text" col="col-xl-6" placeholder="name"/>
                                    <x-admin.input required="true" :value="$row->email" name="email" label="email" type="email" col="col-xl-6" placeholder="email"/>
                                    <x-admin.input name="password" label="password" type="password" col="col-xl-6" placeholder="password"/>
                                    <x-admin.input name="password_confirmation" label="password_confirmation" type="password" col="col-xl-6" placeholder="password_confirmation"/>
                                    <x-admin.input required="true" :value="$row->roles[0]->id ?? ''" name="role_id" label="role.index" type="select" col="col-xl-6" placeholder="role.index" :options="$roles"/>
                                </div>
                            </div>
                            <div class="tab-pane fade" id="v-pills-media" role="tabpanel" aria-labelledby="v-pills-media-tab" tabindex="0">
                                <div class="row g-3">
                                    <x-admin.file :files="$row->getMedia('profile')" name="profile" class="col-6" :multiple="false"/>
                                </div>
                            </div>
                        </div>
