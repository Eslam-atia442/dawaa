<table class="datatables-products table">
    ...
    @foreach ($rows as $row)
        <tr class="delete_row">
            ...
            <td>
                <span class="text-truncate d-flex align-items-center">{{ @$row->region->name}}</span>
            </td>
            <td>
                <span class="text-truncate d-flex align-items-center">{{ @$row->region->country->name}}</span>
            </td>
            <td>
                <div class="d-inline-block text-nowrap">
                    @can('update-city')
                        <a href="{{ route('admin.cities.edit', ['city' => $row->id]) }}" class="btn btn-sm btn-icon"><i
                                class="text-primary ti ti-edit"></i></a>
                    @endcan
                    <a href="{{ route('admin.cities.show' , ['city' => $row->id])  }}" class="btn btn-sm btn-icon"><i
                            class="text-info ti ti-eye-check"></i></a>
                    @can('delete-city')
                        <a class="btn btn-sm btn-icon delete-row"
                           data-url="{{ route('admin.cities.destroy', $row->id)  }}"><i
