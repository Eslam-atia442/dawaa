...
<table class="datatables-products table">
    ...
    @foreach ($rows as $row)
        <tr class="delete_row">
            ...
            <td>
                <span class="text-truncate d-flex align-items-center">{{ $row->translated_name}}</span>
            </td>

            <td>
                <x-admin.toggle
                    checked="{{$row->is_active}}"
                    url="{{ route('admin.role-toggle',['role' => $row->id ,'key' => 'is_active']) }}">
                </x-admin.toggle>
            </td>
            ...
            <td>
                <div class="d-inline-block text-nowrap">
                   @can('update-role')
                    <a href="{{ route('admin.roles.edit', ['role' => $row->id]) }}" class="btn btn-sm btn-icon"><i class="text-primary ti ti-edit"></i></a>
                   @endcan
                    <a href="{{ route('admin.roles.show' , ['role' => $row->id])  }}" class="btn btn-sm btn-icon"><i class="text-info ti ti-eye-check"></i></a>
                   @can('delete-role')
                    <a class="btn btn-sm btn-icon delete-row" data-url="{{ route('admin.roles.destroy', $row->id)  }}"><i class="text-danger ti ti-trash-x"></i></a>
                   @endcan
                </div>
            </td>
