<table class="datatables-products table">
    ...
    @foreach ($rows as $row)
        <tr class="delete_row">
            ...
            <td>
                <span class="text-truncate d-flex align-items-center">{{ @$row->country->name}}</span>
            </td>

            <td>
                <div class="d-inline-block text-nowrap">
                    @can('update-region')
                        <a href="{{ route('admin.regions.edit', ['region' => $row->id]) }}" class="btn btn-sm btn-icon"><i
                                class="text-primary ti ti-edit"></i></a>
                    @endcan
                    <a href="{{ route('admin.regions.show' , ['region' => $row->id])  }}" class="btn btn-sm btn-icon"><i
                            class="text-info ti ti-eye-check"></i></a>
                    @can('delete-region')
                        <a class="btn btn-sm btn-icon delete-row"
                           data-url="{{ route('admin.regions.destroy', $row->id)  }}"><i
